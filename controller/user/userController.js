  ...
  try {
    ...
    if (personRoleId === 1) {
      ...
      if (role_id === 3 && (!isChild || !parent_id)) {
        ...
        // Admin logic
    if (personRoleId === 2) {
      if (role_id !== 3) return res.status(403).json({ status: 403, message: "Admins can only create sub-users" });
    ...
    if (personRoleId === 3) return res.status(403).json({ status: 403, message: "Sub-users cannot create users" });
    ...
    if (isChild && !parent_id) return res.status(400).json({ status: 400, message: "Parent ID required for sub-user" });
    if (need_user_limit && !sub_user_limit) return res.status(400).json({ status: 400, message: "Sub-user limit required" });
    if (isChild && need_user_limit) return res.status(400).json({ status: 400, message: "Sub-users cannot have user limits" });
    ...
    if (isChild && parent_id) {
      ...
      if (!parentUser) return res.status(404).json({ status: 404, message: "Parent user not found" });
